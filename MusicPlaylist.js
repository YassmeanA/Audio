...
setTimeout(() => {document.querySelector(".Loader").remove();Pages.forEach(Page => {Page.style.opacity="1";});},500);});
...
if(document.querySelectorAll(".mask")[i].querySelector("span").offsetWidth >= 0.75*document.querySelectorAll(".mask")[0].offsetWidth){document.querySelectorAll(".mask")[i].querySelector("span").style.animation=`scroll-text ${document.querySelectorAll(".mask")[i].querySelector('span').offsetWidth / 30}s linear infinite`;}else{document.querySelectorAll(".mask")[i].querySelector("span").style.animation="none";document.querySelectorAll(".mask")[i].querySelector("span").style.transform="none";};
  ...
  const rightText1 = isMouse && isShortScreen ? "78px" : "70px";
  const rightText2 = isMouse && isShortScreen ? "68px" : "60px";

  // Slide 1
  document.querySelectorAll(".listContainer.num1 .item").forEach(item => {
    item.querySelector("img").style.right = rightImg;
    item.querySelector(".title").style.right = rightText1;
  });

// Slide 2
document.querySelectorAll(".listContainer.num2 .item").forEach(item => {
  item.querySelector("img").style.right = rightImg;
  item.querySelector("h3").style.right = rightText1;
});

 // Slide 3
 document.querySelectorAll(".listContainer.num3 .item").forEach(item => {
  item.querySelector(".disk").style.right = rightImg;
  item.querySelector("h3").style.right = rightText1;
});


// Slides 4
document.querySelectorAll(".listContainer.num4 .item").forEach(item => {
  item.querySelector("img").style.right = rightImg;
  item.querySelector("h3").style.right = rightText2;
});
...
document.querySelectorAll(".listContainer.num2 img").forEach(Img => {
  Img.addEventListener("click", () => {
    Img.classList.add("selected");
    SingerInput.value = ""; // Reset so re-selecting the same image works
    SingerInput.click();
  });
  });

  document.querySelectorAll(".listContainer.num3 .disk").forEach(Img => {
    Img.addEventListener("click", () => {
      Img.classList.add("selected");
      SingerInput.value = ""; // Reset so re-selecting the same image works
      SingerInput.click();
...
SingerInput.addEventListener("change", (e) => {
  const file = e.target.files[0];
  if (file && file.type.startsWith("image/")) {
    const reader = new FileReader();
    reader.onload = () => {
      ...
      document.querySelectorAll(".listContainer.num2 img").forEach((Img,index) => {
  
      if(document.querySelectorAll(".listContainer.num2 img")[index].classList.contains("selected")){document.querySelectorAll(".listContainer.num2 img")[index].src = reader.result;}
      Img.classList.remove("selected");
    });

  } else if(Sections[2].classList.contains("active")) {

  document.querySelectorAll(".listContainer.num3 .disk").forEach((Img,index) => {
  
      if(document.querySelectorAll(".listContainer.num3 .disk")[index].classList.contains("selected")){document.querySelectorAll(".listContainer.num3 .disk img")[index].src = reader.result;}
      Img.classList.remove("selected");
    });
...
Item.querySelectorAll("img").forEach(Img => {
  ...
  Item.querySelectorAll(".disk").forEach(Img => {
    Img.addEventListener("click", () => {
      Img.classList.add("selected");
      SingerInput.value = ""; // Reset so re-selecting the same image works
      SingerInput.click();
...
InsertSave.addEventListener("click", () => {
  ...
  if (editingElement) {
    ...
    if(Sections[1].classList.contains("active") || Sections[3].classList.contains("active")) {
